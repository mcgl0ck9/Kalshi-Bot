...
import { fetchPolymarketMarketsWithPrices } from './fetchers/_legacy/polymarket-onchain.js';

// P0 Data Sources (no API keys required)
import { fetchAllSportsOddsESPN, findESPNEdge } from './fetchers/_legacy/espn-odds.js';
import { fetchWastewaterData, fetchFluData as fetchCDCFluData, analyzeWastewaterEdge } from './fetchers/_legacy/cdc-surveillance.js';
import { fetchFundingRates, fetchFearGreedIndex, analyzeFundingEdge, analyzeFearGreedEdge } from './fetchers/_legacy/crypto-funding.js';
import { fetchGDPNow, fetchInflationNowcast, analyzeGDPEdge, analyzeInflationEdge } from './fetchers/_legacy/fed-nowcasts.js';
...
export async function runPipeline(bankroll: number = BANKROLL): Promise<PipelineResult> {
  ...
  try {
    ...
    try {
      const { discoverProfitableWallets, getWhaleDiscoveryStats } = await import('./fetchers/_legacy/whale-discovery.js');
    ...
    try {
      const { fetchAllOptionsImplied, findOptionsEdge } = await import('./fetchers/_legacy/options-implied.js');
